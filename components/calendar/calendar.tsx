  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={goToday}
            className="rounded-md px-2 py-1 text-[13px] font-medium text-slate-600 transition-colors hover:bg-slate-100"
          >
            Today
          </button>
          <button
          ...
          return (
            <button
              ...
              className={cn(
                ...
                c.inMonth ? "text-slate-700" : "text-slate-300",
                isSelected ? "bg-brand-50 ring-1 ring-brand-500" : "hover:bg-slate-100",
              )}
            >
              <span
                className={cn(
              ...
              {evs.length ? (
                <span className="mt-0.5 flex gap-0.5">
                  {evs.slice(0, 3).map((e) => (
                    <span key={e.id} className="h-1.5 w-1.5 rounded-full bg-brand-500" />
                  ))}
                </span>
              ) : null}
            </button>
      ...
      <div className="mt-4 border-t border-slate-100 pt-4">
        <p className="mb-2 text-[13px] font-medium text-slate-700">{formatDateKey(selected)}</p>
        {selectedEvents.length === 0 ? (
          <p className="text-[13px] text-slate-400">No interviews scheduled.</p>
        ) : (
          <ul className="space-y-2">
            {selectedEvents.map((e) => (
              ...
              >
                ...
                <span className="flex-1 truncate text-sm text-slate-700">{e.label}</span>
                {e.link ? (
                  <a
                    href={e.link}
                    target="_blank"
